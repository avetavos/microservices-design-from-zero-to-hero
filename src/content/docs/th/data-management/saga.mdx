...
- [Database per Service](/microservices/th/data-management/database-per-service/) — ขอบเขตที่ทำให้ saga จำเป็น
- [Event Sourcing](/microservices/th/data-management/event-sourcing/) — วิธีที่น่าเชื่อถือในการ publish event ที่ saga พึ่งพา
- [CQRS](/microservices/th/data-management/cqrs/) — มักใช้เพื่อ query สถานะกลางที่ saga สร้างขึ้น

| ข้อดี | ข้อแลกเปลี่ยน |
|---|---|
| จัดการ distributed transaction โดยไม่ต้องใช้ 2-phase commit | eventual consistency — state อาจไม่ consistent ชั่วคราว |
| service ยังคง autonomous — ไม่มี global lock | compensating transaction ซับซ้อน โดยเฉพาะถ้า fail กลางคัน |
| ทนต่อ partial failure — rollback ได้ผ่าน compensating event | debug ยากเมื่อ saga ล้มเหลว — ต้อง trace ข้าม service |
| scale ได้ดี — ไม่มี global coordinator lock ทั้งระบบ | idempotency ของทุก step เป็นสิ่งจำเป็น |

## ข้อผิดพลาดที่พบบ่อย

**Saga ที่ไม่ Idempotent** — step ใน saga ทำซ้ำไม่ได้โดยปลอดภัย
อาการ:
- retry ทำให้เกิด double charge ใน payment
- compensating transaction ทำงานผิดเพราะ state ไม่คาดเดาได้

**Saga ที่ยาวเกินไป** — saga มีมากกว่า 7-10 step
อาการ:
...
> 💡 ตัวอย่างจากของจริง
>
> **microservices.io (Chris Richardson):** เอกสาร pattern ต้นฉบับใช้ตัวอย่าง e-commerce order ที่ต้องตรวจสอบวงเงินเครดิตของลูกค้าข้าม database คนละตัว อธิบายทั้งแบบ choreography (service ยิง event ใส่กัน) และ orchestration (มี orchestrator สั่งการ) ([microservices.io: Saga pattern](https://microservices.io/patterns/data/saga.html))
>
> **รูปแบบทั่วไปที่พบในระบบสั่งอาหาร/delivery และระบบจองที่พัก:** มักโมเดล flow เป็น saga ทำนอง validate order → charge payment → notify merchant → assign fulfillment พร้อม compensating step (refund, release reservation, send cancellation notice) เมื่อ step ใด step หนึ่งล้มเหลวกลางทาง — เป็นภาพประกอบหลักการทั่วไป ไม่ได้อ้างอิงระบบจริงของบริษัทใดบริษัทหนึ่งโดยเฉพาะ
...
export const quizSaga = [
  ...
  },
  {
    ...
    options: [
      'สั่ง ROLLBACK ของ database บน service ปลายทาง',
      'รัน compensating transaction ที่เป็น local transaction ที่กลับด้านเชิงความหมาย',
      'ลบ row ตรง ๆ ผ่าน SQL',
      'ยกเลิกอะไรไม่ได้เลยเมื่อ commit ไปแล้ว',
    ],
    answer: 1,
  },
  {
