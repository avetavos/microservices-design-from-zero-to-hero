...
> 💡 ตัวอย่างจากของจริง
>
> **Kubernetes:** มี probe สามแบบ — `livenessProbe` restart container เมื่อ fail, `readinessProbe` หยุด route traffic เมื่อ fail แต่ไม่ restart, และ `startupProbe` ให้เวลา startup ที่นานพอก่อนที่ probe อื่นจะเริ่มทำงาน ([Kubernetes Docs: Configure Liveness, Readiness and Startup Probes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/))
>
> **Netflix Eureka:** ใช้ health check ของแต่ละ instance เป็นสัญญาณให้ Service Discovery — instance ที่ report ไม่ healthy จะถูก deregister ออกจาก registry ([Spring Cloud Netflix Docs: Eureka health checks](https://docs.spring.io/spring-cloud-netflix/docs/current/reference/html/))
